...
When a search is initiated, a results sidebar opens to display the search results. Results may be sorted in increasing or decreasing order based on RMSD (for pharmacophore searches), similarity score (for shape searches), molecular weight, or number of rotatable bonds. Clicking on a result displays the query-aligned compound in the molecular viewer.  If desired, a new Pharmit session can be initiated from the ligand-protein complex represented by the selected hit compound.
